...
describe("<AlertAck />", () => {
  ...
  it("handles exceptions in alerts fetch", async () => {
    fetchMock.mock(
      {
        name: "alerts",
        url: "/groupAlerts.json",
      ...
      {
        throws: new Error("mock fetch error"),
      },
      {
        overwriteRoutes: true,
      }
  ...
  it("uses faExclamationCircle after failed alerts fetch", async () => {
  ...
  it("uses faExclamationCircle after failed POST", async () => {
    fetchMock.mock(
      {
        name: "silence",
        url: "end:/api/v2/silences",
